"""
Whitened Similarity Dialog

Provides interface for running whitened similarity analysis on hyperspectral data.
Implements reference similarity mapping in whitened space using background statistics.
"""
...
class WhitenedSimilarityDialog(QtWidgets.QDialog):
    """Dialog for configuring and running Whitened Similarity analysis."""
    
    def __init__(self, data_manager, roi_manager, parent=None):
        super().__init__(parent)
        self.data_manager = data_manager
        self.roi_manager = roi_manager
        self.setWindowTitle('Whitened Similarity Analysis')
        self.setModal(True)
        self.resize(500, 400)
    ...
    def _setup_ui(self):
        ...
        similarity_layout.addWidget(QtWidgets.QLabel('Similarity Function:'))
        self.similarity_combo = QtWidgets.QComboBox()
        self.similarity_combo.addItems(['Exponential (0-1 range)', 'Negative Distance (-∞ to 0)'])
        self.similarity_combo.setCurrentIndex(0)  # Default to exponential
        self.similarity_combo.setToolTip('Exponential: smooth 0-1 similarity, Negative Distance: raw distance values')
        similarity_layout.addWidget(self.similarity_combo)
        options_layout.addLayout(similarity_layout)
        ...
        self.run_button = QtWidgets.QPushButton('Run Whitened Similarity Analysis')
        self.run_button.clicked.connect(self._run_whitened_similarity_analysis)
    ...
    def _browse_output_path(self):
        ...
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save Whitened Similarity Results', '', 
    ...
    def _run_whitened_similarity_analysis(self):
        """Run the Whitened Similarity analysis."""
        try:
            self.run_button.setEnabled(False)
            self.progress_bar.setVisible(True)
            self.progress_bar.setValue(0)
            self.status_label.setText('Starting Whitened Similarity analysis...')
            self.status_label.setStyleSheet('color: blue; font-size: 10px;')
            ...
            dataset_name = self.dataset_combo.currentData()
            roi_id = self.roi_combo.currentData()
            similarity_function = self.similarity_combo.currentIndex()  # 0=exponential, 1=negative distance
            ...
            # Run Whitened Similarity calculation
            self.status_label.setText('Computing background statistics...')
            self.progress_bar.setValue(15)
            QtWidgets.QApplication.processEvents()
            
            whitened_result = self._compute_whitened_similarity(data_handler, reference_spectrum, 
                                                              active_bands, similarity_function)
            ...
            if self.memory_radio.isChecked():
                ...
                if not output_path:
                    ...
                    output_path = f"whitened_similarity_{dataset_name}_{timestamp}.hdr"
            ...
            self.result_data = {
                'similarity_map': whitened_result,
            ...
            self.status_label.setText('Whitened Similarity analysis completed successfully!')
            ...
            QtWidgets.QMessageBox.critical(self, 'Whitened Similarity Analysis Error', 
                                         f'Failed to run Whitened Similarity analysis:\n{str(e)}')
    ...
    def _compute_whitened_similarity(self, data_handler, reference_spectrum, active_bands=None, similarity_function=0):
        """Compute Whitened Similarity analysis."""
        shape = data_handler.shape
        rows, cols, bands = shape
        
        # Initialize result array
        similarity_result = np.zeros((rows, cols), dtype=np.float32)
        
        self.status_label.setText('Computing background statistics (mean spectrum)...')
        QtWidgets.QApplication.processEvents()
        
        # Step 1: Compute background statistics - collect all spectra
        all_spectra = []
            ...
            for col in range(cols):
                try:
                    ...
                    if pixel_spectrum is not None:
                        ...
                        all_spectra.append(pixel_spectrum)
                except:
                    continue
                    
                processed_pixels += 1
                if processed_pixels % (total_pixels // 20) == 0:  # Update every 5%
                    progress = 15 + int((processed_pixels / total_pixels) * 20)  # 15-35% range
        ...
        # Compute mean spectrum μ_img
        self.status_label.setText('Computing mean spectrum...')
        QtWidgets.QApplication.processEvents()
        mean_spectrum = np.mean(all_spectra, axis=0)
        self.progress_bar.setValue(40)
        
        # Compute covariance matrix Σ_img
        self.status_label.setText('Computing covariance matrix...')
        QtWidgets.QApplication.processEvents()
        centered_spectra = all_spectra - mean_spectrum
        covariance_matrix = np.cov(centered_spectra, rowvar=False)
        self.progress_bar.setValue(50)
        
        # Step 2: Compute whitening transform W = Σ_img^(-1/2)
        self.status_label.setText('Computing whitening transform...')
        QtWidgets.QApplication.processEvents()
        ...
        try:
            ...
            # Compute W = V * Λ^(-1/2) * V^T
            whitening_matrix = eigenvectors @ np.diag(1.0 / np.sqrt(eigenvalues)) @ eigenvectors.T
        except np.linalg.LinAlgError:
            # Fallback to pseudo-inverse if eigendecomposition fails
            whitening_matrix = scipy.linalg.pinv(scipy.linalg.sqrtm(covariance_matrix))
            
        self.progress_bar.setValue(55)
        
        # Step 3: Transform reference spectrum
        reference_whitened = whitening_matrix @ (reference_spectrum - mean_spectrum)
        
        # Step 4: Process each pixel and compute similarity
        self.status_label.setText('Computing whitened similarity map...')
        QtWidgets.QApplication.processEvents()
        ...
        for row in range(rows):
            for col in range(cols):
                try:
                    ...
                    if pixel_spectrum is None:
                        similarity_result[row, col] = np.nan
                        continue
                    ...
                    # Transform pixel to whitened space
                    pixel_whitened = whitening_matrix @ (pixel_spectrum - mean_spectrum)
                    
                    # Compute Euclidean distance in whitened space
                    distance = np.linalg.norm(pixel_whitened - reference_whitened)
                    
                    # Convert to similarity score
                    if similarity_function == 0:  # Exponential (0-1 range)
                        similarity = np.exp(-distance**2)
                    else:  # Negative distance (-∞ to 0)
                        similarity = -distance
                    
                    similarity_result[row, col] = similarity
            ...
            if row % max(1, rows // 25) == 0:  # Update 25 times
                progress = 55 + int((processed_pixels / total_pixels) * 25)  # 55-80% range
                self.progress_bar.setValue(progress)
                QtWidgets.QApplication.processEvents()
